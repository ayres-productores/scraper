...
def sincronizar_archivos():
    ...
    rutas_en_bd = {a.ruta_archivo for a in ArchivoDescargado.query.all()}
    ...
    for ruta in archivos_en_disco:
        if ruta not in rutas_en_bd:
            ...
            try:
                ...
                escaneo = Escaneo.query.filter_by(usuario_id=admin.id).first()
                if not escaneo:
                    escaneo = Escaneo(
                        usuario_id=admin.id,
                        estado='completado',
                ...
                if compania_detectada:
                    ...
                    comp = None
                    if dominio:
                        comp = Compania.query.filter_by(dominio_email=dominio).first()
    ...
    for archivo in ArchivoDescargado.query.all():
