#!/usr/bin/env python
"""
Servidor LAN - Portal de Seguros
Este script inicia el servidor para que sea accesible desde cualquier
dispositivo en la red local (LAN).
"""
...
def setup_environment():
    """Configura el entorno para el ejecutable."""
    ...
    for folder in ['archivos_usuarios', 'uploads', 'downloads']:
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        format=log_format,
        handlers=[
            logging.FileHandler(os.path.join(logs_dir, 'servidor.log'), encoding='utf-8'),
...
def main():
    ...
    banner = f"""
{'=' * 60}
       PORTAL DE SEGUROS - SERVIDOR LAN
...
{'=' * 60}
  ...
  IMPORTANTE:
  - Asegurese de que el firewall permita conexiones al puerto 5000
...
"""
    ...
    try:
        ...
        msg = "  [*] Iniciando servidor con Waitress (produccion)..."
        print(msg)
        logger.info("Iniciando con Waitress")
        print()
        serve(app, host='0.0.0.0', port=port, threads=4)
    except ImportError:
        # Si no esta waitress, usar el servidor de desarrollo de Flask
        print("  [!] Waitress no instalado, usando servidor de desarrollo Flask")
        print("  [!] Para mejor rendimiento, instale: pip install waitress")
        logger.info("Iniciando con Flask (desarrollo)")
        print()
        app.run(
            host='0.0.0.0',
            port=port,
